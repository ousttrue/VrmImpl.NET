...
namespace VrmImpl.SceneGraph.Bvh
{
    public class BvhNode
    {
        ...
        {
            Offset = ParseOffset(r.ReadLine() ?? throw new BvhException("no line"));

            Channels = ParseChannel(r.ReadLine() ?? throw new BvhException("no line"));
        }
        ...
        private static BvhChannel[] ParseChannel(string line)
        {
            ...
            return split
                .Skip(2)
                .Select(x => (BvhChannel)Enum.Parse(typeof(BvhChannel), x))
                .ToArray();
        }
