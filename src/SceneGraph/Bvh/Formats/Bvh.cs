namespace UniHumanoid
{
    public class Bvh
    {
        ...
        public bool TryGetPathWithPropertyFromChannel(
            BvhChannelCurve channel,
            out PathWithProperty pathWithProp
        )
        {
        ...
        public override string ToString()
        {
            return string.Format(
                "{0}nodes, {1}channels, {2}frames, {3:0.00}seconds",
                Root.Traverse().Count(),
                Channels.Length,
                m_frames,
        ...
        public static Bvh? Parse(string src)
        {
            using (var r = new StringReader(src))
            {
                ...
                {
                    var frameSplit = (r.ReadLine() ?? throw new BvhException("no line")).Split(':');
                    ...
                    var frameTimeSplit = (r.ReadLine() ?? throw new BvhException("no line")).Split(
                        ':'
                    );
                ...
                {
                    var line = r.ReadLine() ?? throw new BvhException("no line");
        ...
        {
            var firstline = (r.ReadLine() ?? throw new BvhException("no line")).Trim();
            ...
            if ((r.ReadLine() ?? throw new BvhException("no line")).Trim() != "{")
            {
